// Ported from langchain-ai/agent-chat-ui (src/hooks/use-file-upload.tsx).
...
export function useFileUpload({
  ...
  useEffect(() => {
    ...
    // Element-specific handlers (textarea wrapper)
  ...
  const handlePaste = async (
    ...
    const isDuplicatePasted = (file: File) => {
      if (file.type === "application/pdf") {
    ...
    const duplicateFiles = validFiles.filter(isDuplicatePasted);
    const uniqueFiles = validFiles.filter((file) => !isDuplicatePasted(file));
    if (invalidFiles.length > 0) {
      toast.error(
